...
export default function SettingsClient({ profile }: { profile: Profile }) {
  ...
  return (
    ...
    >
      ...
      <div className="mb-5 flex flex-wrap gap-1 rounded-xl border border-[#e8e3dd] bg-white p-1">
        {TABS.map((t) => (
          <button
            key={t.k}
            onClick={() => setTab(t.k)}
      ...
      <div className="card-pad">
        ...
        <div className="mt-3 flex flex-wrap gap-5 rounded-xl bg-ink-50 p-4 text-sm font-semibold text-ink-700">
          <label className="flex items-center gap-2">
            <input type="checkbox" className="h-4 w-4 accent-lavender-500" checked={f.is_subgroup} onChange={(e) => setF({ ...f, is_subgroup: e.target.checked })} />
          ...
          <label className="flex items-center gap-2">
            <input type="checkbox" className="h-4 w-4 accent-sand-500" checked={f.is_elective} onChange={(e) => setF({ ...f, is_elective: e.target.checked })} />
            Сонгон судлах
          </label>
        </div>
      </div>
          ...
          <tbody className="divide-y divide-[#f1ece6]">
            {classes.map((c) => (
              <tr key={c.id} className="hover:bg-ink-50/40">
                ...
                <td className="td">
                  <select
                    className="rounded-lg border border-[#e8e3dd] px-2 py-1 text-xs font-semibold"
                    value={c.shift}
                    onChange={(e) =>
                      run(
                        () =>
          ...
          <div key={d.shift} className="card-pad">
              ...
              <div className="flex items-center gap-3">
                <span
                  className={`grid h-9 w-9 place-items-center rounded-xl text-sm font-black ${
                    s.active ? "bg-lavender-500 text-white" : "bg-ink-100 text-ink-400"
                  }`}
                >
                  {d.shift}
                </span>
            ...
            <div className="mt-4 flex flex-wrap gap-1.5">
              {times.map((t) => (
                <span
                  key={t.period}
                  className="rounded-lg border border-[#e8e3dd] bg-ink-50 px-2.5 py-1 text-[11px] font-semibold text-ink-600"
                >
                  <b className="text-lavender-600">{t.period}</b> · {t.start}–{t.end}
                </span>
              ))}
            </div>
